...
# DES encrypt
class DES:
    def __init__(self, key, coding='cp1251', n_bit=8):
        # const tables
    ...
    def __init_keys(self):
        bits = bitarray()
        bits.frombytes(self.key.encode(self.coding))
    ...
    def encode(self, message: bytes, forward=True) -> bytes:
        ...
        if forward:
            padding_length = self.n_bit - (len(message) % self.n_bit)
            message = message + padding_length * chr(padding_length).encode()

        result = b''
        for b in range(0, len(message), self.n_bit):
            # 8 byte block processing
            data = message[b:b + self.n_bit]
            bits = bitarray()
            bits.frombytes(data)
